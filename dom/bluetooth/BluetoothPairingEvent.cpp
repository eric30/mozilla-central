...
BluetoothPairingEvent::Create(const nsString& aDeviceObjectPath, const PRUint32& aPasskey)
{
  nsRefPtr<BluetoothPairingEvent> event = new BluetoothPairingEvent();

  event->mPasskey = aPasskey;
  event->mDeviceObjectPath = aDeviceObjectPath;
...
NS_IMETHODIMP
BluetoothPairingEvent::GetPin(PRUint32* aPin)
{
  *aPin = mPin;
  return NS_OK;
}
...
NS_IMETHODIMP
BluetoothPairingEvent::GetDeviceObjectPath(nsAString& aDeviceObjectPath)
{
  aDeviceObjectPath = mDeviceObjectPath;
  return NS_OK;
}
